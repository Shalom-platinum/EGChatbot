...
import { isAppError } from "@/app-model/errors";
import {
  convertFilesToDataUris,
  createAttachmentMetadata,
} from "../utils/fileAttachments";
import { parseSseLine, splitSseBuffer } from "../utils/sseParser";
import { AppAction, IChatItem } from "@/app-model/appState";

// SEND MESSAGE;
// GET MESSAGES
// GET CONVERSATION ID
// GET METADATA ABOUT AGENT (BACKEND)
...
export class ChatService {
  ...
  /**
   * Refresh runtime dependencies while keeping stream lifecycle state.
   */
  configure(
    apiUrl: string,
    getAccessToken: () => Promise<string | null>,
  ...
  private async ensureAuthToken(): Promise<string> {
    ...
    if (!token) {
      console.error(
        "[ChatService] Token acquisition failed: getAccessToken returned null",
      );
      throw createAppError(new Error("Failed to acquire access token"), "AUTH");
    }
    console.log(
      "[ChatService] Token acquired successfully, length:",
      token.length,
    );
    return token;
  }
  ...
  async sendMessage(
    ...
    try {
      ...
      const { content, imageDataUris, fileDataUris, attachments } =
