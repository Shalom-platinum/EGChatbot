...
import { AgentCreateSession, AgentMetaResponse, AgentNewSessionResponse } from "@/app-model/api";
...
const api = createApi({
  ...
  tagTypes: ["Post"],
  endpoints: (build) => ({
    getAgentMetadata: build.query<AgentMetaResponse, void>({
      query: (body) => ({ url: `/agent/metadata` }),
    }),

      createNewSession: build.mutation<AgentNewSessionResponse, AgentCreateSession>({
      query: (body) => ({ url: `/sessions/visitor`, body: body, method: 'POST' }),
    }),
  }),
});
