...
import { IAnnotation, IChatItem } from "@/app-model/chat";
import { parseContentWithCitations } from "@/utils/citationParser";
import { useFormatTimestamp } from "@/utils/useFormatTimestamp";
import { Dock, Folder, Globe, UserCircle } from "lucide-react";
...
function AssistantMessageComponent({
  ...
  const timestamp = message.more?.time
  ...
  const indexedCitations = useMemo(() => {
    ...
    if (message.annotations && message.annotations.length > 0) {
      const seen = new Map<
        string,
        { index: number; annotation: IAnnotation; count: number }
      >();
  ...
  const handleCitationClick = useCallback(
    (index: number, annotation?: IAnnotation) => {
  ...
  const renderCitation = (
    ...
    const getIcon = () => {
      switch (annotation.type) {
        case "uri_citation":
          return <Globe className={styles.citationIcon} />;
        case "file_path":
          return <Folder className={styles.citationIcon} />;
        default:
          return <Dock className={styles.citationIcon} />;
      }
    };
      ...
      >
        ...
        >
          ...
          <span className={styles.citationContent}>
            ...
            {count > 1 && (
              <span className={styles.citationCount}>×{count}</span>
            )}
            {isClickable && <a className={styles.citationExternalIcon} />}
          </span>
        </span>
      </Tooltip>
    );
  ...
  return (
    <div
      key={message.id}
      className={`flex items-end gap-2 ${props.botBubbleWidth}`}
    >
      <UserCircle/>
      ...
      <div className="flex flex-col gap-1.5 min-w-0">
        <div className="bg-white rounded-2xl rounded-bl-sm px-4 py-2.5 shadow-sm">
          <p
            className={`text-gray-800 whitespace-pre-line leading-relaxed ${props.isFull ? "text-sm" : "text-sm"}`}
          >
            <Markdown content={message.content} />
          </p>
        </div>
        {message?.["type"] === "options" && (
          <p>Listing Options Here;</p>
  ...
  //     disclaimer={<span>Reliance Bot</span>}
  //     footnote={
